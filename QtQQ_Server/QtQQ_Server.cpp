...
void QtQQ_Server::initComBoxData()
{
	...
	int depCounts = queryDepModel.rowCount() - 1; //“公司群”这条记录不能算公司分支部门
...
void QtQQ_Server::updateTableData(int depID, int employeeID)
{
	...
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < columns; j++)
		{
			...
			QString strData = m_queryInfoModel.data(index).toString(); //获取i行j列的值 
...
void QtQQ_Server::on_logoutBtn_clicked()
{
	...
	else
	{
		...
		QMessageBox::information(this, QString::fromLocal8Bit("提示"),
			QString::fromLocal8Bit("员工%1 的企业QQ:%2已被注销")
			   .arg(strName)
			   .arg(employeeID));
...
void QtQQ_Server::on_addBtn_clicked()
{
	...
	{
		QMessageBox::information(this, QString::fromLocal8Bit("提示"),
			QString::fromLocal8Bit("请选择员工头像的路径！"));
		return;
	}
